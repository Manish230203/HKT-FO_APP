...
import { Moon, Plus, Trash2, ChevronDown, Building, X, Clock, MapPin, Camera } from 'lucide-react-native';
  ...
  const loadClientsAndSites = async () => {
    try {
      ...
      if (templates && templates.length > 0 && templates[0].questions?.length > 0) {
        ...
        qList = DEFAULT_NIGHT_MANDATORY_QUESTIONS;
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setAlertInfo({
        ...
        message: 'Night Visit Report submitted successfully and site visit session checked out.',
        type: 'success',
      });
    } catch (e: any) {
