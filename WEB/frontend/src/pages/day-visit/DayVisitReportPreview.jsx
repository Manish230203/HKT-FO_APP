...
import { getStoredVisitReports } from "../night-visit/mockData";
import html2pdf from "html2pdf.js";
import { toast } from "sonner";

export function DayVisitReportTemplate({ report, hideTitle = false, hideLogo = false, hideHeader = false }) {
  ...
  const resolvePhotoUrl = (url) => {
    ...
    if (url.startsWith("/uploads/") || url.startsWith("uploads/")) {
      ...
      return `http://${host}:8002${clean}`;
    }
    return url;
  };

  const parsedChecklist = Array.isArray(report?.checklist)
    ? report.checklist
    : (typeof report?.checklist === "string" ? (JSON.parse(report.checklist || "[]") || []) : []);
  const checklistPhotos = parsedChecklist.map((c) => c.photo || (Array.isArray(c.photos) ? c.photos[0] : null)).filter(Boolean);

  const rawPhotos = report?.photos || report?.photo_evidence || report?.photoEvidence || [];
  const basePhotosList = Array.isArray(rawPhotos) ? rawPhotos : (typeof rawPhotos === "string" ? (JSON.parse(rawPhotos || "[]") || []) : []);
  const photosList = Array.from(new Set([...basePhotosList, ...checklistPhotos])).filter(Boolean);
  ...
  const reportIdStr = report?.formattedReportId || `${report?.reportIndex || "02"}-${clientName}-${unit}-ODV-${formatDateToDMY(visitDate)}`;
  ...
  const checklistList = rawChecklist.filter((c) => {
    const statusStr = (c.status || c.answer || "").toString().trim();
    const obsStr = (c.observation || c.remarks || "").toString().trim();
    const actionStr = (c.correctiveAction || "").toString().trim();
    return statusStr !== "" || obsStr !== "" || actionStr !== "";
  });
  const obsList = parseArray(report?.observations, []);

  let customerFeedbackText = report?.customerFeedback;
  let overallSuggestionsText = report?.suggestions;
  if (report?.suggestions && !report?.customerFeedback) {
    try {
    ...
    <div className="report-template-content bg-white text-slate-900 mb-0 p-2">
        ...
        <div className="border-b-2 border-[#1e3a8a] pb-4 mb-4">
          {!hideLogo && (
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h2 className="text-sm font-black tracking-tight text-slate-900 leading-none">{compName}</h2>
              </div>
            </div>
          )}
          <div className="flex justify-between items-start gap-4">
            <div className="flex flex-col min-w-0 shrink max-w-[65%]">
              {!hideTitle && (<h1 className="text-sm font-black text-[#1e3a8a] leading-tight uppercase tracking-tight w-fit">FIELD OFFICER DAY VISIT REPORT</h1>)}
              <span className="text-[10px] font-bold text-slate-800 mt-1 uppercase font-mono break-all">REPORT ID : {reportIdStr}</span>
            </div>
            <div className="text-right text-[10px] font-bold text-slate-800 leading-normal uppercase font-mono shrink-0 whitespace-nowrap">
              DATE : {formatDateToDMY(visitDate)} {startTime} - {endTime}
            </div>
          </div>
        </div>
      )}
      ...
      <div className="mb-6 pdf-avoid-card">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 p-4 border border-t-0 border-slate-200 bg-slate-50/70 rounded-b-lg">
          ...
          <div><label className="block text-[7px] font-semibold text-slate-500 uppercase tracking-wider mb-0.5">Shift</label><span className="text-[13px] font-extrabold text-slate-900">{report?.shift || "Day Shift"}</span></div>
          <div><label className="block text-[7px] font-semibold text-slate-500 uppercase tracking-wider mb-0.5">Visit Type</label><span className="text-[13px] font-extrabold text-slate-900">Day Visit</span></div>
      ...
      <div className="mb-6">
        <div className="bg-[#1e3a8a] text-white text-[11px] font-extrabold uppercase tracking-wider px-4 py-2 rounded-t-lg border-l-4 border-indigo-400 pdf-section-title">A. Pre-defined Checklist Answers</div>
        <div className="border border-t-0 border-slate-200 rounded-b-lg">
          {checklistList.length > 0 ? (
            <div className="w-full text-xs text-left">
              <div className="bg-slate-200 border-b border-slate-300 text-slate-900 font-bold grid grid-cols-12 gap-2 p-2">
                <div className="col-span-1 text-center">Sr No</div>
                <div className="col-span-4">Inspection Point</div>
                <div className="col-span-3">Observation</div>
                <div className="col-span-2 text-center">Status</div>
                <div className="col-span-2">Corrective Action</div>
              </div>
              <div className="divide-y divide-slate-200">
                {checklistList.map((c, idx) => {
                  const st = (c.status || c.answer || "-").toString().trim();
                  ...
                  return (
                    <div key={idx} className="grid grid-cols-12 gap-2 py-3 px-2 items-center pdf-avoid-card">
                      <div className="col-span-1 text-center font-medium text-slate-600">{idx + 1}</div>
                      <div className="col-span-4 font-semibold text-slate-900 break-words">{c.question}</div>
                      <div className="col-span-3 text-slate-700 break-words">{c.observation || c.remarks || "-"}</div>
                      <div className={`col-span-2 text-center font-bold ${colorClass}`}>{st}</div>
                      <div className="col-span-2 text-slate-700 break-words">{c.correctiveAction || "-"}</div>
                    </div>
                  );
                })}
              </div>
            </div>
          ) : (<div className="p-4 text-xs text-slate-500 italic">No answered checklist items recorded for this visit.</div>)}
        </div>
      </div>

      <div className="mb-6 pdf-avoid-card pdf-page-break pt-6">
        <div className="bg-[#1e3a8a] text-white text-[11px] font-extrabold uppercase tracking-wider px-4 py-2 rounded-t-lg border-l-4 border-indigo-400 pdf-section-title">B. On-Spot Custom Observations</div>
        <div className="border border-t-0 border-slate-200 rounded-b-lg">
          {obsList.length > 0 ? (
            <div className="w-full text-xs text-left">
              <div className="bg-slate-200 border-b border-slate-300 text-slate-900 font-bold grid grid-cols-12 gap-2 p-2">
                <div className="col-span-1 text-center">Sr No</div>
                <div className="col-span-2">Inspection Point</div>
                <div className="col-span-2">Observation</div>
                <div className="col-span-2">Action Required</div>
                <div className="col-span-2">Action Done / Status</div>
              ...
              <div className="divide-y divide-slate-200">
                {obsList.map((obs, idx) => (
                  <div key={idx} className="grid grid-cols-12 gap-2 py-3 px-2 items-center pdf-avoid-card">
                    <div className="col-span-1 text-center font-medium text-slate-600">{idx + 1}</div>
                    <div className="col-span-2 font-semibold text-slate-900 break-words">{obs.actionPoint || obs.inspectionPoint || "—"}</div>
                    <div className="col-span-2 text-slate-700 break-words">{obs.observation || "—"}</div>
                    <div className="col-span-2 text-slate-700 break-words">{obs.actionRequired || "—"}</div>
                    <div className="col-span-2 text-slate-700 break-words">{obs.actionDone || "—"}</div>
                    <div className="col-span-1 text-slate-700 break-words">{obs.correctiveMeasures || "—"}</div>
                    <div className="col-span-2 text-slate-700 break-words">{obs.remarks || "—"}</div>
                  </div>
                ))}
              </div>
            </div>
          ) : (<div className="p-4 text-xs text-slate-500 italic">No custom observations recorded for this visit.</div>)}
        </div>
      </div>

      <div className="mb-6 pdf-avoid-card">
        <div className="bg-[#1e3a8a] text-white text-[11px] font-extrabold uppercase tracking-wider px-4 py-2 rounded-t-lg border-l-4 border-indigo-400 pdf-section-title">Customer Feedback</div>
        <div className="flex flex-col justify-center border border-t-0 border-slate-200 rounded-b-lg p-3 bg-slate-50 text-xs text-slate-700 font-medium min-h-[80px] whitespace-pre-wrap shadow-sm break-words">{(customerFeedbackText || "No customer feedback recorded.").trim()}</div>
      </div>

      <div className="mb-6 pdf-avoid-card">
        <div className="bg-[#1e3a8a] text-white text-[11px] font-extrabold uppercase tracking-wider px-4 py-2 rounded-t-lg border-l-4 border-indigo-400 pdf-section-title">Overall Suggestions</div>
        <div className="flex flex-col justify-center border border-t-0 border-slate-200 rounded-b-lg p-3 bg-slate-50 text-xs text-slate-700 font-medium min-h-[80px] whitespace-pre-wrap shadow-sm break-words">{(overallSuggestionsText || "No overall suggestions recorded.").trim()}</div>
      </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {photosList.map((photo, pIdx) => (
                <div key={pIdx} className="aspect-square rounded-lg overflow-hidden border border-slate-300 bg-slate-100 flex items-center justify-center shadow-2xs">
                  <img src={resolvePhotoUrl(photo)} alt={`Evidence ${pIdx + 1}`} className="w-full h-full object-cover" onError={(e) => { e.target.style.display = 'none'; }} />
                </div>
              ))}
            </div>
          ) : (
...
export default function VisitReportPreview() {
  ...
  const formattedReportId = `${reportIndex}-${clientName}-${report.unit}-ODV-${formatDateToDMY(report.visitDate)}`;
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto pb-12 print:p-0 print:m-0 print:max-w-full">
        ...
        <div className="flex items-center gap-2">
          {report?.emailAccess === 1 || report?.email_access === 1 ? (
            <Button
              onClick={() => {
                setRecipientEmail("");
                setEmailSubject(`Day Visit Report - ${report?.unit || ""}`);
                setEmailMessage(`Dear Team,\n\nPlease find the Day Visit Report for site: ${report?.unit || ""}.\n\nBest regards,\nField Officer Management`);
      ...
      >
        <DayVisitReportTemplate report={fullReportData} />
      </div>
